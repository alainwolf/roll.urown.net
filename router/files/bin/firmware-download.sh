...
#  3 - Failed to download sha256sums.asc file
...
echo "================================"
download "sha256sums.asc" "$gpgsig_url" || {
...
missing_key=$(runpgp --status-fd 1 --with-fingerprint --verify \
	"sha256sums.asc" "sha256sums" 2>/dev/null | sed -ne 's!^.* NO_PUBKEY !!p')
...
runpgp --with-fingerprint --verify "sha256sums.asc" "sha256sums" || {
